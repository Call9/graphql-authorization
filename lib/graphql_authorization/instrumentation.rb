  ...
  def instrument(type, field)
    ...
    new_resolve_proc = lambda do |obj, args, ctx|
      unless ctx[:ability] == :root
        raise GraphqlAuthorization::Unauthorized, "not authorized to execute #{fieldType.name}" unless ctx[:ability].canExecute(fieldType, toSymKeys(args.to_h)) || @always_allow_execute
        raise GraphqlAuthorization::Unauthorized, "not authorized to access #{field.name} on #{type.name}" unless ctx[:ability].canAccess(type, field.name.to_sym, obj, toSymKeys(args.to_h))
      end
